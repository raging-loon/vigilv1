...
static void* verify_ip_addr(void * args){
  ...
  printf("[DEBUG] aiaoic_args = %s\n",(char *)temp_info->__ip_addr);

  for(int i = 0; i <= ip_addr_stat_counter_len; i++){
    if(strcmp((char *)temp_info->__ip_addr , ip_stats[ip_addr_stat_counter_len].ip_addr) == 0){
      ip_stats[i].count++;
      // free(temp_info);
      goto end; 
    }
  ...
  strcpy(ip_stats[ip_addr_stat_counter_len].ip_addr,(char *)temp_info->__ip_addr);
  ip_stats[ip_addr_stat_counter_len].count++;
  printf("[DEBUG] temp_info->__ip_addr = %s ip_stats[ip_addr_stat_counter_len].ip_addr = %s\n",(char *)temp_info->__ip_addr,ip_stats[ip_addr_stat_counter_len].ip_addr);
  // free(temp_info);
end:;
}
